...
// resolveLayout calculates memory layout and collects labels and constants.
func (a *Assembler) resolveLayout(tokens []lexer.Token) ([]AssembledData, error) {
	...
	for {
		...
		case PlusToken, MinusToken:
			if tokenPosition != 1 {
				return nil, fmt.Errorf("[layout] unexpected token '%s'", t.Literal)
			}
		...
		case MnemonicToken:
			...
			if err != nil {
				return nil, fmt.Errorf("%s at %d:%d: %w", t.Literal, t.SourceLine, t.SourceColumn, err)
			}
		...
		case IdentifierToken:
			if tokenPosition == 1 {
				if directiveID, found := a.bareDirective(t.Literal); found {
					...
					if directiveID != EndDirective || !a.canStartStatementAfterLabel(nextToken) {
						if err := a.preprocessDirectiveID(directiveID, asmTokens, advanceProgramCounter, finalizeCurrentSegment); err != nil {
							return nil, err
						}
						if a.programCounter != currentSegmentStart+uint16(currentSegmentSize) {
							currentSegmentStart = a.programCounter
			...
			if nextToken.ID == EqualsSymbolToken {
				err := a.processConstantAssignment(t, asmTokens, true)
				if err != nil {
					return nil, err
				}
				tokenPosition = 0
			} else if tokenPosition == 1 {
				if !a.canStartStatementAfterLabel(nextToken) {
					return nil, fmt.Errorf("[layout] unexpected identifier '%s'", t.Literal)
				}
				err := a.recordLabelAddress(t)
				if err != nil {
					return nil, err
	...
	if err := conditionals.Complete(); err != nil {
		return nil, err
	}
...
func (a *Assembler) canStartStatementAfterLabel(t lexer.Token) bool {
	if isTerminatorToken(t.ID) {
		return true
	}
	switch t.ID {
	case MnemonicToken, PeriodToken, AsterixSymbolToken:
		return true
	case IdentifierToken:
		_, found := a.bareDirective(t.Literal)
		return found
	default:
		return false
	}
...
func (a *Assembler) preprocessDirective(asmTokens *Tokens, advanceProgramCounter func(int), finalizeSegment func()) error {
	...
	if nextToken.ID == IdentifierToken {
		...
		if directiveID, found := a.directives[strings.ToUpper(directiveName)]; found {
			...
			asmTokens.Next()
			return a.preprocessDirectiveID(directiveID, asmTokens, advanceProgramCounter, finalizeSegment)
		}
	}
	return nil
}

func (a *Assembler) preprocessDirectiveID(directiveID int, asmTokens *Tokens, advanceProgramCounter func(int), finalizeSegment func()) error {
	...
	case DsDirective:
		...
		advanceProgramCounter(size)
	case VarDirective:
		err := a.processVarDirective(asmTokens, true)
		if err != nil {
			return err
		}
	case EndDirective:
...
func (a *Assembler) calculateByteDirectiveSize(asmTokens *Tokens) (int, error) {
	...
	for {
		...
		asmTokens.Next()
		if _, err := a.EvaluateExpression(asmTokens, "", true); err != nil {
			return 0, err
		}
		size++
	}
...
func (a *Assembler) calculateDataSpaceDirectiveSize(asmTokens *Tokens) (int, error) {
	...
	sizeValue, err := a.EvaluateExpression(asmTokens, "", true)
	if err != nil {
		return 0, fmt.Errorf("[calculateDataSpaceDirectiveSize] %w", err)
	}
	if sizeValue < 0 {
		return 0, fmt.Errorf("[calculateDataSpaceDirectiveSize] negative size: %d", sizeValue)
	}

	return int(sizeValue), nil
}

// layoutLabelSizer determines operand size for forward-referenced labels during layout resolution.
// It makes assumptions based on the mnemonic and whether it has Relative addressing modes
func (a *Assembler) layoutLabelSizer(mnemonic string) (string, any, error) {
	...
	if !ok {
		return "", nil, fmt.Errorf("[Assembler layoutLabelSizer] unknown mnemonic '%s'", mnemonic)
	}
...
func (a *Assembler) processConstantAssignment(identifierToken lexer.Token, asmTokens *Tokens, preprocess bool) error {
	...
	evaluatedValue, err := a.EvaluateExpression(asmTokens, "", preprocess)
	if err != nil {
